...
static uint8_t gbufLCDIndexed[6] = {10, 7, 4, 18, 21, 27};
static uint8_t DateTimeIndexValid[6] = {59, 59, 23, 31, 11, 99};
/*
...
static void ShowEditTime(uint8_t T) {
    ...
    lcdwritebuf(1, 2, SystemDateBuf, PRINT_BUF_DATE_lENGTH);
    if (T) {
        gu8LCDBlink ^= 1;
...
static void keyNumbersEvents(uint8_t s) {
    if ((gu8TimeIndexValue == -1) || (gu8TimeIndexValue == 4))
        return;
    ...
    if (gu8TimeIndexValue > 2) {
        ValueIndex = gu8TimeIndexValue + 1;

    } else {
        ValueIndex = gu8TimeIndexValue;
    }
    Byte = *(Date + ValueIndex);
    print2d(Byte, s2, 0);
    s2[gu8LCDBlink] = s;
    Byte = conv2d(s2);
    Valid = (DateTimeIndexValid[gu8TimeIndexValue]);
    if (Byte <= Valid) {
        *(Date + ValueIndex) = Byte;
        ShowEditTime(1);
    }
}
